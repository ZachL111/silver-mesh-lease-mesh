...
`silver-mesh-lease-mesh` is centered on distributed systems work in SQL. The technical target is: Implement an SQL distributed systems project for lease storage recovery, using log and snapshot fixtures and replay consistency checks.
...
The score is deterministic. The current threshold is 171. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 6.
